...
## 2026-06-02 — npx 엔트리포인트 감지 복구

1. `src/server.ts`의 `isMain` 판단 로직을 `OMAKASE_FORCE=true` 환경 변수 방식에서 `process.argv[1].includes("claude-omakase")` 포함 여부 체크 방식으로 되돌렸다.
2. 변경된 파일은 `src/server.ts` 하나이며, `38485ad` 커밋이 `OMAKASE_FORCE` 플래그 방식으로 교체했던 내용을 `c999491` 커밋의 원래 접근법으로 복구한 것이다.

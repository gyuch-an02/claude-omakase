...
export async function handle(args: z.infer<typeof recommendInput>) {
  const [catalog, profile] = await Promise.all([load(), profileLib.load()]);
  ...
  // If there's no signal at all, surface verified entries we know are broadly
  // useful. We avoid hard-coding ids — just take the top verified entries by
  // tag breadth.
  ...
  return {
    ...
    recommendations: candidates.map((e) => ({
      id: e.id,
      name: e.name,
      type: e.type,
